...
def Class GameDateTime():
  def __init__(self, value):
    ...
    self.__time_chars = '0123456789abcdefghijklmnopqrstuvwxyz'
    self.__days_per_month = {i: 30 for i in range(1, 13)}
    self.__days_per_month[1] = 32
    self.__days_per_month[7] = 32
    total_days = sum(self.__days_per_month.values())
    self.__total_weeks = (total_days + 6) // 7
    ...
    if isinstance(value, list):
      ...
      if len(value)!= 4:
        ...
        self.day   = Char2Num[ts[0]]
        self.month = Char2Num[ts[1]]
        self.year  = Char2Num[ts[2]] + 1200 - 15
        self.hour  = Char2Num[ts[3]]
    else:
  ...
  def Char2Num(chars='0123456789abcdefghijklmnopqrstuvwxyz'):
  ...
  def CalcCalendarDay(self):
    ...
    for i in range(1, self.month):
      self.calendar_day+=self.days_per_month[i]
  ...
  def GetSeason(self):
    if week < 12 or (self.week == 12 and self.weekday == 1):
      return 'winter'
    elif week < 20 or (self.week == 20 and self.weekday <= 5):
      return 'spring'
    elif week < 29 or (self.week == 29 and self.weekday <= 4):
      return 'summer'
    elif week < 42 or (self.week == 42 and self.weekday <= 3):
      return 'fall'
    else:
      return 'winter'
  ...
  def __add__(self, days):
    ...
    for _ in range(days):
      result.day += 1
      if result.day > self.days_per_month[result.month]:
        result.day = 1
        result.month += 1
        if result.month > 12:
          result.month = 1
          result.year += 1
    result.CalcCalendarDay()
    result.CalcWeek()
    result.CalcWeekDay()
